...
const EXPECTED_CATALOG_OUTPUT_TYPE = 'numeric'; // Check1 -> 'star'.
const EXPECTED_PRIMARY_SCORE_VALUE = 25; // Check2 -> 26.
const EXPECTED_REOPENED_ANNOTATION_AVG = 25; // Check3 -> 26.
...
type Score = { id: string; label_id: string; source_type: string; trace_id: string; tracer_project_id: string;
  ...
  value: { value: number }; value_history: unknown[]; deleted: number; deleted_us: string | null; created_us: string; updated_us: string };
...
type NativeScore = { id: string; source_type: string; source_id: string; label_id: string; label_name: string; label_type: string;
  label_settings: Wire; label_allow_notes: boolean; value: { value: number }; value_history: unknown[]; score_source: string;
...
test('DASH-E2E-010: a saved trace annotation widget retains numeric scores', {
  tag: ['@flow'],
  annotation: flowAnnotation({ id: 'DASH-E2E-010', area: 'dashboards',
    userGoal: 'A saved trace annotation widget retains numeric scores.',
    steps: [
      'submit three native numeric trace Scores in independently seeded primary, sibling and foreign projects',
      'create and name a Table widget with 7D and Day',
      'discover the numeric annotation and Traces metrics and explicitly choose Average and Distinct Count',
      'select primary Project through its native exhausted cursor chain and verify scoped unfiltered facts',
      'select numeric Equals25 and the same label breakdown, prove disjoint75 and restore25',
  ...
  const labelName = `${prefix}-numeric`, dashboardName = `${prefix}-dashboard`, widgetName = `${prefix}-widget`;
  ...
  const seeds = [{ key: 'a', project: 0, value: 25 }, { key: 'b', project: 0, value: null },
    { key: 's', project: 1, value: 90 }, { key: 'f', project: 2, value: 95 }].map(seed => ({ ...seed,
  ...
  const linkSql = 'SELECT id,queue_id,label_id,required,deleted FROM model_hub_annotationqueuelabel WHERE queue_id=ANY($1::uuid[]) ORDER BY id';
  const itemSql = `SELECT id,queue_id,source_type,trace_id,observation_span_id,trace_session_id,call_execution_id,
  ...
  await test.step('public telemetry and scoped label/default-queue setup', async () => {
    ...
    for (const owner of [actor, foreignActor]) {
      const input = { name: labelName, type: 'numeric', settings: { min: 0, max: 100, step_size: 1, display_type: 'slider' }, allow_notes: false };
    ...
    expect(sorted(initialLabels.map(({ created_at, updated_at, ...row }) => row))).toEqual(sorted([actor, foreignActor].map((owner, index) => ({
      id: labelIds[index], name: labelName, type: 'numeric', settings: { min: 0, max: 100, step_size: 1, display_type: 'slider' },
    ...
    expect(sorted(initialLinks.map(({ id, ...row }) => row))).toEqual(sorted(queues.map((queue, index) => ({ queue_id: queue.id,
      label_id: labelIds[index === 2 ? 1 : 0], required: false, deleted: false }))));
  ...
  try {
    ...
    }, POLL.SPAN_VISIBLE).toEqual(sorted(seeds.map(seed => ({ id: seed.traceId, project_id: projectIds[seed.project], is_deleted: 0 }))));
  ...
  try {
    ...
    // source_adapters._annotation_definition: both/numeric is not custom number.
    const annotationProperty = (id: string): Property => ({ property_id: `annotation:${id}`, property_kind: 'annotation',
      category: 'annotation_metric', name: id, display_name: labelName, source: 'both', type: 'numeric', output_type: 'numeric',
      role: 'metric', data_type: 'numeric', sources: ['annotation', 'datasets', 'traces'] });
    ...
    const configFor = (value?: number, grouped = false, project: string | null = projectIds[0], label = labelIds[0]): Config => ({
      project_ids: [], time_range: { preset: '7D' }, granularity: 'day', metrics: [
        { id: label, name: labelName, property_id: `annotation:${label}`, display_name: labelName, type: 'annotation_metric',
          source: 'both', aggregation: 'avg', label_id: label, output_type: 'numeric' },
        ...
        ...(value === undefined ? [] : [{ column_id: label, property_id: `annotation:${label}`, display_name: labelName,
          source: 'both', output_type: 'numeric', filter_config: { filter_type: 'number', filter_op: 'equals', filter_value: value, col_type: 'ANNOTATION' } }]),
      ], breakdowns: grouped ? [{ name: label, property_id: `annotation:${label}`, display_name: labelName,
        type: 'annotation_metric', source: 'both', label_id: label, output_type: 'numeric' }] : [],
    });
    const canonical = configFor(25, true), metadata = { query_complete: true, query_exact: false, query_status: 'complete', query_provenance: 'current_property_catalog' };
    ...
    const searchCatalog = async (property: Property, mode: 'metric' | 'filter' | 'breakdown', category: 'All' | 'Traces' | 'Annotations') => {
      ...
      if (property.property_kind === 'annotation') {
        ...
        expect(rows[0].output_type).toBe(EXPECTED_CATALOG_OUTPUT_TYPE);
      }
    ...
    // Own-label group '25' averages25 on Score.created_at; Traces group '25' counts1
    ...
    const assertResult = (receipt: Receipt<QueryBody>, input: Config, oracle: Oracle, reopened = false): number[] => {
      ...
      for (const [index, metric] of result.metrics.entries()) {
        ...
        expect(metric.series.map(series => series.name)).toEqual([oracle.grouped && (index === 0 ? oracle.value : oracle.traces) !== null ? '25' : 'total']);
        ...
        if (reopened && index === 0) expect(series.data.find(point => Date.parse(point.timestamp) === oracle.scoreDay)!.value).toBe(EXPECTED_REOPENED_ANNOTATION_AVG);
      }
      return buckets;
    };
    ...
    const assertTable = async (oracle: Oracle, buckets: number[], tile = false) => {
      const columns: Record<string, (number | null)[]> = {
        [`${labelName}${oracle.grouped && oracle.value !== null ? ' / 25' : ''} (avg)`]: buckets.map(bucket => bucket === oracle.scoreDay ? oracle.value : null),
        [`Traces${oracle.grouped && oracle.traces !== null ? ' / 25' : ''} (count_distinct)`]: buckets.map(bucket => bucket === plantedDay ? oracle.traces : null),
      };
    ...
    const assertControls = async () => {
      ...
      await uiExpect(card(labelName).getByRole('combobox')).toHaveText('Equals'); await uiExpect(card(labelName).getByRole('spinbutton')).toHaveValue('25');
    ...
    try {
      await test.step('1 submit three native numeric trace Scores', async () => {
        for (const seed of [seeds[0], seeds[2], seeds[3]]) {
          ...
          expect(forSource.body!.result).toHaveLength(1); expect(forSource.body!.result[0]).toMatchObject({
            ...
            existing_scores: {}, existing_notes: '', existing_label_notes: {} });
          expect(forSource.body!.result[0].labels).toEqual([{ id: labelId, name: labelName, type: 'numeric',
            settings: { min: 0, max: 100, step_size: 1, display_type: 'slider' }, description: '', allow_notes: false, required: false, order: 1 }]);
          ...
          await uiExpect(heading).toHaveCount(1); const input = heading.locator('../..').getByRole('spinbutton');
          for (const [name, value] of [['min', '0'], ['max', '100'], ['step', '1']]) await uiExpect(input).toHaveAttribute(name, value);
          await input.fill(String(seed.value)); await uiExpect(input).toHaveValue(String(seed.value));
      ...
      await test.step('combined PG definitions and latest PG-to-CH Score barrier', async () => {
        ...
        for (const [index, submission] of submissions.entries()) {
          ...
          expect(fact).toMatchObject({ id: wire.id, label_id: labelIds[seed.project === 2 ? 1 : 0], source_type: 'trace', trace_id: seed.traceId,
            ...
            queue_item_id: wire.queue_item, score_source: 'human', notes: '', value: { value: [25, 90, 95][index] }, value_history: [], deleted: 0, deleted_us: null,
        ...
        expect(sorted(initialItems)).toEqual(sorted(submissions.map(({ seed, receipt }) => ({ id: receipt.body!.result.scores[0].queue_item,
          queue_id: queues[seed.project].id, source_type: 'trace', trace_id: seed.traceId, observation_span_id: null, trace_session_id: null,
        ...
        try {
          ...
          expect(initialScores.find(row => row.trace_id === seeds[0].traceId)!.value.value).toBe(EXPECTED_PRIMARY_SCORE_VALUE);
      ...
      const positive: Oracle = { value: 25, traces: 1, scoreDay: scoreDay(seeds[0].traceId), grouped: true };
      ...
      await test.step('3 discover numeric annotation and Traces with exact scope', async () => {
        ...
        for (const owner of [actor, foreignActor, emptyActor]) {
          ...
          for (const [index, property] of expected.entries()) expect(response.body.result.metrics[index]).toMatchObject(property);
        }
      }, { timeout: UI_READY });
      ...
      await test.step('4 select primary Project and prove positive and empty scopes', async () => {
        ...
        for (const control of [
          { owner: actor, project: projectIds[1], label: labelIds[0], value: 90, traces: 1, day: scoreDay(seeds[2].traceId), name: 'sibling' },
          { owner: foreignActor, project: projectIds[2], label: labelIds[1], value: 95, traces: 1, day: scoreDay(seeds[3].traceId), name: 'foreign' },
      ...
      await test.step('5 choose inline Equals and breakdown, prove disjoint75 and restore25', async () => {
        await queryAfter('equals25', configFor(25), { ...positive, grouped: false }, async () => {
          await chooseProperty(ownProperty); await uiExpect(card(labelName).getByRole('combobox')).toHaveText('Equals');
          await card(labelName).getByRole('spinbutton').fill('25'); await uiExpect(card(labelName).getByRole('spinbutton')).toHaveValue('25');
        });
        await queryAfter('label-breakdown', canonical, positive, () => chooseProperty(ownProperty, true)); await assertControls();
        await queryAfter('disjoint75', configFor(75, true), { ...positive, value: null, traces: null }, () => card(labelName).getByRole('spinbutton').fill('75'));
        await queryAfter('restored25', canonical, positive, () => card(labelName).getByRole('spinbutton').fill('25')); await assertControls();
        // Current numeric VALUES is a finite configured vocabulary, not observed Score values or a native popup.
        for (const control of [
          ...
          expect(response.body.result).toEqual({ values: [], query_complete: true, query_status: 'complete', has_more: false, next_cursor: null, browse_status: 'exhausted' });
        }
      }, { timeout: UI_READY });
