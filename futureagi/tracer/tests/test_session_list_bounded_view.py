...
def test_string_page_public_dispatch_and_old_order_token(org, cursor, kind, preferred):
    ...
    # number or boolean leaf costs its witness scan against an index-only
    # probe before the route is chosen; this double cannot be read either, so
    # that page walks - one EXPLAIN, no witness statement, the walk read
    # once. A page carrying a string leaf (the text, mixed and "number" shapes
    # here, whose number leaf sits beside a string ``in``) is never probed: a
    # row estimate does not price a statement that reads the string map.
    probed = preferred and kind == "boolean"
    expected_reads = int(probed) if preferred else (2 if cursor else 1)
    assert (
